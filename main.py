...
parser.add_argument('--eval_every_epoch',type=int, dest='eval_every_epoch', default=20,
...
def lowlight_test(lowlight_enhance):
    ...
    lowlight_enhance.test(test_low_data, test_high_data, test_low_data_name, save_dir=args.save_dir,
                          decom_flag=args.decom,ckpt_dir=args.ckpt_dir)


def main(_):
    if args.use_gpu:
        # 设置 GPU 设备
        os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # 设置使用的 GPU，"0" 对应的是你当前的 GPU
        ...
        if len(physical_devices) > 0:
            # 获取 GPU 的总内存大小（单位：字节）
            total_memory = 4096  # 4096MB
            memory_limit = int(total_memory * 0.5)  # 设置为 50% 的内存
            ...
            print(f"[*] GPU memory usage set to 50% ({memory_limit // (1024 ** 2)}MB)")
        else:
            print("[!] No GPU found")


        model = LowLightEnhance(args.ckpt_dir)  # Initialize model
        if args.phase == 'train':
